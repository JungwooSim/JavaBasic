...
public class StreamEx7 {
    public static void main(String[] args) {
        ...
        List<Student2> maleStudent = stuBySex.get(true); // 남자 취합
        List<Student2> femaleStudent = stuBySex.get(false); // 여자 취합

        for (Student2 s : maleStudent) System.out.println(s);
        System.out.println(); // 줄갱신
        ...
        Map<Boolean, Student2> topScoreBySex2 = Stream.of(stuArr).collect(
                partitioningBy(Student2::isMale, collectingAndThen(maxBy(Comparator.comparingInt(Student2::getScore)), Optional::get))
        );
        ...
        List<Student2> failedMaleStu = failedStuBySex.get(true).get(true); // 남자 취합
        List<Student2> failedFemaleStu = failedStuBySex.get(false).get(true); // 여자 취합
